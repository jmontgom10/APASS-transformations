...
goodStars = np.logical_not(landoltStars['Vmag_landolt'].data.mask)
goodStars = np.logical_and(
    goodStars,
    landoltStars['e_Vmag_landolt'].data > 0
)
goodStars = np.logical_and(
    goodStars,
    np.logical_not(landoltStars['V-R_landolt'].data.mask))
goodStars = np.logical_and(
    goodStars,
    landoltStars['e_V-R_landolt'].data > 0
)
goodStars = np.logical_and(
    goodStars,
    np.logical_not(landoltStars['g_mag_apass'].data.mask))
goodStars = np.logical_and(
    goodStars,
    landoltStars['e_g_mag_apass'].data > 0
)
goodStars = np.logical_and(
    goodStars,
    np.logical_not(landoltStars['r_mag_apass'].data.mask)
)
goodStars = np.logical_and(
    goodStars,
    landoltStars['e_r_mag_apass'].data > 0
)
...
Vmag_landolt   = landoltStars['Vmag_landolt'].data.data
e_Vmag_landolt = landoltStars['e_Vmag_landolt'].data.data
VR_landolt     = landoltStars['V-R_landolt'].data.data
e_VR_landolt   = landoltStars['e_V-R_landolt'].data.data
g_mag_apass    = landoltStars['g_mag_apass'].data.data
e_g_mag_apass  = landoltStars['e_g_mag_apass'].data.data
r_mag_apass    = landoltStars['r_mag_apass'].data.data
e_r_mag_apass  = landoltStars['e_r_mag_apass'].data.data

# Compute the R-magnitude
Rmag_landolt   = Vmag_landolt - VR_landolt
e_Rmag_landolt = np.sqrt(e_Vmag_landolt**2 + e_VR_landolt**2)

# Compute the abscicca value and uncertainty
Rl_ra   = Rmag_landolt - g_mag_apass
e_Rl_ra = np.sqrt(e_Rmag_landolt**2 + e_r_mag_apass**2)

# Compute the ordinate value and uncertainty
ga_ra   = g_mag_apass - r_mag_apass
e_ga_ra = np.sqrt(e_g_mag_apass**2 + e_r_mag_apass**2)
...
# import and setup the ODR fittter
from scipy import odr

# Build the data
data = odr.RealData(x=ga_ra, y=Rl_ra, sx=e_ga_ra, sy = e_Rl_ra)

# Build the model
lineFunc = lambda B, x: B[0]*x + B[1]
model = odr.Model(lineFunc)

# Build the fitter and run it
odr = odr.ODR(data, model, beta0=[-1.0, 0.0])
output = odr.run()

# Establish the boundaries of acceptable parameters for the prior
bounds = [tuple(np.arctan(output.beta[0]) + np.array([-0.1, 0.1])), # Theta (angle of the line slope)
          (-1, +1),     # b_perp (min-dist(line-origin))
...
plotBounds = [(-0.86, -0.82), # Theta (angle of the line slope)
              (-0.115, -0.075)] # b_perp (min-dist(line-origin))
...
def ln_like_bg(params, x, y, sx, sy):
    ...
    data_cov_matrix_list = [build_cov_matrix(sx1, sy1, 0) for sx1, sy1 in zip(sx, sy)]

    # Convolve the two gaussians (simply add their covairance matrices)
    conv_cov_matrix = [out_cov_matrix + data_cov_mat for data_cov_mat in data_cov_matrix_list]
    # conv_cov_matrix = map(lambda dcm: (out_cov_matrix + dcm), data_cov_matrix_list)
...
# Now build a table to store fitted parameters and covariance matrix values
fitValTable = Table(
...
data          = (ga_ra, Rl_ra, e_ga_ra, e_Rl_ra)
...
ax2.axis((-1.0, 2.5, -3.0, 1.0))
ax2.autoscale(False)

# Plot the slope +/- sigma_intrinsic lines
# Start by including a "typical errorbar"
ax2.errorbar(-0.5, -1.75, xerr=np.median(data[2]), yerr=np.median(data[3]), color='k')
...
ax2.set_xlabel("$g_a - r_a$")
ax2.set_ylabel("$R_l - r_a$")

# Construct the figure name
fig2Name = figName + '_MCMC{0:d}.pdf'.format(iLoop+1)
