...
goodStars = np.logical_and(
    goodStars,
    np.logical_not(landoltStars['Vmag_apass'].data.mask))
goodStars = np.logical_and(
    goodStars,
    landoltStars['e_Vmag_apass'].data > 0
)
goodStars = np.logical_and(
    goodStars,
    np.logical_not(landoltStars['B-V_apass'].data.mask)
)
goodStars = np.logical_and(
    goodStars,
    landoltStars['e_B-V_apass'].data > 0
)
...
Vmag_apass     = landoltStars['Vmag_apass'].data.data
e_Vmag_apass   = landoltStars['e_Vmag_apass'].data.data
BV_apass       = landoltStars['B-V_apass'].data.data
e_BV_apass     = landoltStars['e_B-V_apass'].data.data

# Compute the abscicca value and uncertainty
Vl_Va   = Vmag_landolt - Vmag_apass
e_Vl_Va = np.sqrt(e_Vmag_landolt**2 + e_Vmag_apass**2)
...
ax2.set_xlabel("$(B - V)_a$")
ax2.set_ylabel("$V_l - V_a$")
